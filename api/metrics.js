...
module.exports = async (req, res) => {
  // 1. PRIORITAS UTAMA: Handle Strava Callback (Jika ada ?code= di URL)
  ...
  if (query && query.code) {
    try {
      ...
      const client = await pool.connect();
      // Simpan User & Nama
      await client.query("INSERT INTO connected_platforms (user_id, platform_name, display_name) VALUES ($1, 'strava', $2) ON CONFLICT (user_id) DO UPDATE SET display_name = $2", [uid, athleteName]);

      const actRes = await axios.get('https://www.strava.com/api/v3/athlete/activities?per_page=100', { headers: { 'Authorization': `Bearer ${access_token}` } });
      for (const act of actRes.data) {
        ...
        await client.query("INSERT INTO activities (user_id, activity_id, title, distance, moving_time, total_elevation_gain, tss, start_date, type) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)", [uid, act.id.toString(), act.name, act.distance, act.moving_time, act.total_elevation_gain, tss, act.start_date, act.type]);
      }
      
      await calculateMetrics(uid);
      client.release();
      
      // TENDANG BALIK KE DASHBOARD (Gak nyangkut lagi)
      return res.redirect('/');
    } catch (e) {
  ...
  // 3. DEFAULT: AMBIL DATA DASHBOARD (GET)
  try {
    const client = await pool.connect();
    ...
    const activities = await client.query("SELECT * FROM activities ORDER BY start_date DESC LIMIT 10");
    ...
    const prs = await client.query("SELECT MAX(distance) as max_dist, MAX(total_elevation_gain) as max_elev, MIN(CASE WHEN distance BETWEEN 9900 AND 10100 THEN moving_time END) as best_10k FROM activities");

    client.release();
    return res.status(200).json({ metrics: result.rows, activities: activities.rows, weekly: weekly.rows, prs: prs.rows[0] });
  } catch (e) { return res.status(500).json({ error: e.message }); }
};
